...
execute at @s[scores={blood=40}] run effect give @e[distance=0..2,type=!#shield_hero:undead] instant_damage 1 50 true
execute at @s[scores={blood=40}] run effect give @e[distance=0..2,type=#shield_hero:undead] instant_health 1 5 true
execute at @s[scores={blood=40}] run data modify entity @e[limit=1,distance=0..2,type=!#blutopher:bone_entities,tag=!bluto] Health set value 0
execute at @s[tag=aj.blutopher.root,scores={blood=5..40}] run tp @e[tag=!shield,type=!#shield_hero:exclude,distance=0..2] ~ ~0.5 ~
